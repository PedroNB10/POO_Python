...
class CtrlTurma():       
    ...
    def insereAluno(self, event):
        alunoSel = self.limiteIns.listbox.get(tk.ACTIVE) # valor selecionado
        ...
        self.limiteIns.listbox.delete(tk.ACTIVE) # remove o item da listbox
        
    def mostraTurmas(self):
        msg = ''

        for turma in self.listaTurmas:
            msg += 'Codigo: ' + turma.codigo + '\n' +'Disciplina: '+ turma.disciplina.codigo + '\n'
            msg += 'Estudantes' + '\n' 
            for est in turma.estudantes:
                msg += est.nroMatric + ' -- ' + est.nome + '\n'

            msg += '-----------\n'

        self.limiteLista = LimiteMostraTurmas(msg)
